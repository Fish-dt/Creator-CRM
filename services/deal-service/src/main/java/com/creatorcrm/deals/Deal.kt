...
interface DealRepository : JpaRepository<Deal, UUID> {
    ...
    fun findAllByCreatorIdAndStatus(creatorId: UUID, status: DealStatus): List<Deal>
...
data class DealCreatedEvent(val dealId: UUID, val creatorId: UUID, val brandName: String, val value: BigDecimal, val occurredAt: Instant = Instant.now())
data class DealStatusChangedEvent(val dealId: UUID, val from: DealStatus, val to: DealStatus, val occurredAt: Instant = Instant.now())
...
class DealService(
    ...
    fun create(cmd: CreateDealCommand): Deal {
        ...
        kafka.send("deal.created", saved.id.toString(), DealCreatedEvent(saved.id, saved.creatorId, saved.brandName, saved.value))
        return saved
    }

    fun transition(id: UUID, newStatus: DealStatus): Deal {
...
data class TransitionRequest @JsonCreator constructor(val status: DealStatus)
...
class DealController(private val svc: DealService) {
    ...
    @PostMapping
    fun create(@Valid @RequestBody cmd: CreateDealCommand) = svc.create(cmd)

    @GetMapping("/{id}")
    fun get(@PathVariable id: UUID) = svc.findById(id)

    @GetMapping
    fun list(@RequestParam creatorId: UUID) = svc.findByCreator(creatorId)

    @PatchMapping("/{id}/status")
    fun transition(@PathVariable id: UUID, @RequestBody req: TransitionRequest) =
        svc.transition(id, req.status)

    @GetMapping("/earnings")
    fun earnings(@RequestParam creatorId: UUID) = mapOf("total" to svc.totalEarnings(creatorId))
}
